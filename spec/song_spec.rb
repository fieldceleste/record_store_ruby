...
describe '#Song' do
  ...
  before(:each) do
    Album.clear()
    Song.clear()
    @album = Album.new("Giant Steps", nil, "2020", "pop", "chee")
    @album.save()
  end
  ...
  describe('#==') do
    it("is the same song if it has the same attributes as another song") do
      ...
      song2 = Song.new("Naima", @album.id, nil, "song lyrics", "Sally")
  ...
  describe('.all') do
    it("returns a list of all songs") do
      song = Song.new("Giant Steps", @album.id, nil, "song lyrics 2", "Bob")
      song.save()
      song2 = Song.new("Naima", @album.id, nil, "song lyrics", "Sally")
      song2.save()
  ...
  describe('#update') do
    it("updates an song by id") do
      song = Song.new("Naima", @album.id, nil, "song lyrics", "Sally")
      song.save()
      song.update("Mr. P.C.", @album.id, "song lyrics 3", "Joe")
  ...
  describe('.find_by_album') do
    it("finds songs for an album") do
      album2 = Album.new("Blue", nil, "2000", "pop", "chee")
      album2.save
      song = Song.new("Naima", @album.id, nil, "song lyrics", "Bob")
      song.save()
      song2 = Song.new("California", album2.id , nil, "song lyrics", "Sally")
      song2.save()
